...
public function userRegistration($name,$email,$pwd,$phone,$nic,$elcType,$region){
  ...
  if($nor>0){
    ...
    $userId = $autoNumber -> NumberGeneration("user_id","user_tbl","PNT");

    //create token to activate account
    $token=bin2hex(random_bytes(15));

    //insert data to user table
    $sqlInsert = "INSERT INTO user_tbl VALUES('$userId','$name','$email','$elcType','$region','$phone','$nic','$token',0);";
...
function editdata($id,$name,$email,$elcType,$region,$phone,$nic){

  $update1 = "UPDATE user_tbl SET user_name='$name', user_email='$email',ElectracityCat='$elcType',region='$region', user_phone='$phone', user_nic='$nic' WHERE  user_id='$id' AND d_status = 0;";
     //lets check the errors 
      if($this->dbResult->error){
      echo($this->dbResult->error);
      exit;
